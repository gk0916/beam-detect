...
# category_set = set({'Crack','Spallation','Efflorescence','ExposedBars','CorrosionStain'})
# class_indices = {'Crack':0,'Spallation':1,'Efflorescence':2,'ExposedBars':3,'CorrosionStain':4}
...
def parser_info(info: dict, only_cat=True, class_indices=None):
    ...
    if "object" in info['annotation']:
        for obj in info['annotation']['object']:
            ...
            # obj_name = obj['name']
            defect_dict = obj['Defect']
            for k,v in defect_dict.items():
                if int(v) == 1 and k in class_indices:
                    obj_name = k
            ...
            #     continue
                    ...
                    print(f'filename========{filename}')
                    import subprocess
                    subprocess.call(f"copy images\\{filename} exposed_tendons\\images\\",shell=True)
...
def parseXmlFilse(voc_dir, save_dir):
    ...
    # class_indices = dict((v, k) for k, v in enumerate((category_set)))
